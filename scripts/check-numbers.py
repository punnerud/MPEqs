...
CHECKS = [
    ...
    # The spec gate and its ablation: lie-freedom priced against coverage.
    ("data/custom/specgate.json", lambda d: float(d["planted_caught"]),
     29.0, 0.001, "planted mis-mappings the gate caught of 30"),
    ("data/custom/specgate.json", lambda d: float(d["aime"]["wrong"]),
     0.0, 0.001, "wrong deliveries on the AIME arm where every mapping was wrong"),
    ("data/custom/gateablate.json",
     lambda d: float(d["aime"]["none"]["wrong"]),
     8.0, 0.001, "lies the ungated pipeline would have told on the AIME arm"),
    ("data/custom/gateablate.json", lambda d: float(d["aime"]["both"]["wrong"]),
     0.0, 0.001, "lies under the full composition — the only zero row"),
    ("data/custom/gateablate.json", lambda d: float(d["battery"]["both"]["right"]),
     3.0, 0.001, "what lie-freedom gives back on the easy battery: three of thirty"),
    ("data/custom/gateablate.json", lambda d: float(d["battery"]["echo"]["right"]),
     13.0, 0.001, "value-echo alone, the best single layer, at no model cost"),
    # MPEqs on GSM8K: an honest loss, and cross-kind agreement that works.
    ("data/custom/gsmsolve.json", lambda d: float(d["solo"]),
     28.0, 0.001, "GSM8K solved by the 35B answering directly"),
    ("data/custom/gsmsolve.json", lambda d: float(d["m35_exact"]),
     21.0, 0.001, "solved by mapping onto exact machinery — the machinery loses here"),
    ("data/custom/gsmsolve.json", lambda d: float(d["m1b_ran"]),
     3.0, 0.001, "1B specs that even executed, of 16 that parsed"),
    ("data/custom/gsmsolve.json", lambda d: float(d["shape_repairs"]),
     3.0, 0.001, "specs where the slots overruled the name"),
    ("data/custom/gsmgate.json", lambda d: float(d["both"][1]),
     15.0, 0.001, "cross-kind gate deliveries, all right"),
    ...
    # Base arithmetic, approximation, word counting, primes.
    ("data/custom/bands5.json", lambda d: float(d["mpeqs"]),
     16.0, 0.001, "a complete sweep of the newest four classes"),
    ("data/custom/bands5.json", lambda d: float(d["solo"]),
     5.0, 0.001, "and what the model managed alone"),
    ...
    # Five rewritten exemplars, same model: the ceiling was the example.
    ("data/custom/grandmix_1b_tpl.json", lambda d: float(d["mpeqs"]),
    ...
    ("data/custom/tokenprefetch.json", lambda d: float(d["token_coverage"]),
     0.624, 0.005, "expert coverage from a per-token prefetch table at 25% residency"),
    ...
    ("data/custom/tokenprefetch.json", lambda d: float(d["delta"]),
     0.097, 0.005, "what knowing the word is worth, once the tokeniser matches"),
    ("data/custom/ctxprefetch.json",
     lambda d: float(d["self_overlap"]["n1_layer8"][0]),
     0.58, 0.02, "a word's expert overlap with itself, against 0.22 for random pairs"),
    ("data/custom/ctxprefetch.json",
     lambda d: float(d["self_overlap"]["n4_layer8"][0]),
    ...
    # Scaling the trace: the lookup-able part of routing saturates.
    ("data/custom/scaleprefetch.json",
     lambda d: float(d["small"]["coverage"]["composed"]),
     0.678, 0.005, "composed coverage from a 4k-position table, the best measured"),
    ("data/custom/scaleprefetch.json",
     lambda d: float(d["big"]["coverage"]["composed"]),
     0.650, 0.005, "the same with fifteen times the data: blurred, not sharpened"),
    ("data/custom/scaleprefetch.json",
     lambda d: float(d["big"]["starved_share"]),
     0.395, 0.005, "unseen-bigram share after scaling, down from 0.724"),
    ("data/custom/scaleprefetch.json",
