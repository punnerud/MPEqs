#!/usr/bin/env python3
"""Named slots instead of free-form expressions: moving the adaptation into the record.

Phase 119 found two classes that resisted better exemplars because they need genuine
ADAPTATION: a conversion whose units are never the example's units, and a fold whose step
expression IS the problem. Both are the same defect seen from the schema side — a slot
that must be composed rather than filled — so both slots were changed:

    convert now accepts the question's own WORDS ("kilometres per hour"), normalised by
      the record, so the model copies rather than translates
    iterate now accepts "multiply" and "add" as named slots, so a fold is two numbers
      rather than an expression

Same model, same machines, same battery. This measures whether adaptation can be moved
out of the model and into the schema, which is the study's oldest move applied to its
own interface.
...
def main(per_class=2, k=2, model="olmoe-1b",
         out="data/custom/grandmix_1b_slots.json"):
    ...
    TEMPLATES = [
        ...
        # The units are COPIED from the question's own words — the slot accepts them.
        ("convert", "A train moves at 29 kilometres per hour. Exactly how many "
         "millimetres per second is that? Give a fraction.",
         '{"solver":"convert","value":29,"from":"kilometres per hour",'
         '"to":"millimetres per second"}'),
        ...
        # A fold by NAMED SLOTS: two numbers copied, no expression to compose.
        ("fractions", "Start with 8/5. Nine times in a row, multiply by 3/8 and then "
         "add 5/6. Give the exact result as a fraction.",
         '{"solver":"iterate","init":"8/5","multiply":"3/8","add":"5/6","from":1,'
         '"to":9}'),
    ...
    summary = {"n": n, "model": model, "qwen_reference": 33, "baseline_1b": 18, "template_1b": 23,
               "bank": len(bank),
