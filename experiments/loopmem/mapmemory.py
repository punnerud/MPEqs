#!/usr/bin/env python3
"""A retrieved bank of exemplar mappings instead of three fixed ones.
...
BANK = [
    ...
    ("factorial_divisors", "The product of all positive divisors d of 10! for which "
     "10!/d is a perfect cube can be written as a power product of primes. Find the "
    ...
    ("modular_inverse", "For each n let b(n) be the least positive multiple of 7 that "
     "is congruent to 1 modulo 3^n. How many n at most 200 satisfy b(n) = b(n+1)?",
     '{"solver":"multisearch","variables":[{"name":"n","from":1,"to":200}],'
     '"conditions":["inv_mod(7, 3**1 * pow2(0) * 3**0) == inv_mod(7, 3**1)"],'
     '"aggregate":"count"}'),
...
def main(k=2, out="data/custom/mapmemory.json"):
    k = int(k)
