...
def _domains():
    """The domain machines live in solvers3; imported here so run2 still dispatches
    across the whole library and no caller has to know about the split."""
    try:
        from solvers3 import SOLVERS3
    except ImportError:
        return {}
    return SOLVERS3
...
def _call(name, spec):
    ...
    fn = SOLVERS2.get(name) or _domains().get(name) or SOLVERS1.get(name)
    if fn is None:
...
def main(out="data/custom/solvers2.json"):
    ...
    total_solvers = len(SOLVERS2) + len(_domains()) + 15
